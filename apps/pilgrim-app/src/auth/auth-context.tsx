...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const logout = useCallback(async () => { const refreshToken = session.tokens?.refreshToken; if (refreshToken) await logoutFromApi(refreshToken).catch(() => undefined); await clearAuthSession(); setSession(emptyAuthSession); router.replace('/(auth)/login'); }, [router, session.tokens?.refreshToken]);
  ...
  const updateProfile = useCallback(async (displayName: string) => { const user = await updateAuthProfile({ displayName }); const nextSession = { ...session, user }; await saveAuthSession(nextSession); setSession(nextSession); }, [session]);
