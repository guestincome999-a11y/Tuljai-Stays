...
type AuthResponseWithOnboarding = {
  session: AuthSession['activeSession'];
...
async function setOnboardingRequired(required: boolean) {
  if (!(await SecureStore.isAvailableAsync())) return;
  if (required) await SecureStore.setItemAsync(ONBOARDING_KEY, 'true');
  else await SecureStore.deleteItemAsync(ONBOARDING_KEY);
}
async function isOnboardingRequired() {
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    async function restore() {
      ...
      if (restored.tokens?.refreshToken) {
        ...
        if (refreshed) {
          const nextSession = {
            ...restored,
            tokens: { ...restored.tokens, accessToken: refreshed.accessToken },
          };
  ...
  const routeAfterLogin = useCallback(
    async (response: AuthResponseWithOnboarding) => {
      const required = response.onboardingRequired === true;
      await setOnboardingRequired(required);
      router.replace(required ? '/(auth)/onboarding' : '/(app)/home');
    },
    [router],
  );
  const signInWithOtp = useCallback(
    async (phoneNumber: string, otp: string) => {
      const response = (await verifyLoginOtp(phoneNumber, otp)) as AuthResponseWithOnboarding;
  ...
  const logout = useCallback(async () => {
    ...
    if (refreshToken) await logoutFromApi(refreshToken).catch(() => undefined);
    await clearAuthSession();
    await setOnboardingRequired(false);
    setSession(emptyAuthSession);
  ...
  const refreshSession = useCallback(async (): Promise<string | null> => {
    const refreshToken = session.tokens?.refreshToken;
    if (!refreshToken || !session.tokens) return null;
    const refreshed = await refreshAccessToken(refreshToken).catch(() => null);
    if (!refreshed) return null;
    const nextSession: AuthSession = {
      ...session,
      tokens: { ...session.tokens, accessToken: refreshed.accessToken },
    };
    await saveAuthSession(nextSession);
    setSession(nextSession);
