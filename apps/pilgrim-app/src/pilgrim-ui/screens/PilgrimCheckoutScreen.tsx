...
function PriceRow({ label, value }: { label: string; value: string }) { return <View className="mt-4 flex-row justify-between gap-4"><Text className="flex-1 text-sm text-warm-500">{label}</Text><Text className="text-sm font-bold text-warm-900">{value}</Text></View>; }
function PaymentOption({ active, icon, label, description, onPress }: { active: boolean; icon: 'cash' | 'qrcode-scan'; label: string; description: string; onPress: () => void }) { return <Pressable onPress={onPress} className={`min-h-20 flex-row items-center gap-3 rounded-2xl border bg-white p-4 ${active ? 'border-2 border-saffron-500' : 'border-warm-200'}`}><MaterialCommunityIcons color={active ? ui.saffronDeep : ui.muted} name={icon} size={25} /><View className="flex-1"><Text className="text-base font-extrabold text-warm-900">{label}</Text><Text className="mt-1 text-xs leading-4 text-warm-500">{description}</Text></View><MaterialCommunityIcons color={active ? ui.saffronDeep : ui.muted} name={active ? 'radiobox-marked' : 'radiobox-blank'} size={23} /></Pressable>; }
function GuestCounter({ label, value, min, max, onChange }: { label: string; value: number; min: number; max: number; onChange: (value: number) => void }) { return <View className="flex-row items-center rounded-2xl border border-warm-200 bg-white p-4"><Text className="flex-1 text-base font-extrabold text-warm-900">{label}</Text><View className="flex-row items-center gap-3"><Pressable onPress={() => onChange(Math.max(min, value - 1))} className="h-10 w-10 items-center justify-center rounded-full border border-warm-200"><MaterialCommunityIcons name="minus" color={ui.maroon} size={18} /></Pressable><Text className="w-5 text-center font-extrabold text-warm-900">{value}</Text><Pressable onPress={() => onChange(Math.min(max, value + 1))} className="h-10 w-10 items-center justify-center rounded-full border border-warm-200"><MaterialCommunityIcons name="plus" color={ui.maroon} size={18} /></Pressable></View></View>; }
interface DateOption { date: string; day: string; month: string; weekday: string }
function DateSelector({ label, options, selectedDate, onSelect }: { label: string; options: DateOption[]; selectedDate: string; onSelect: (date: string) => void }) { return <View><Text className="mb-2 text-sm font-bold text-warm-700">{label}</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>{options.map((option) => { const selected = option.date === selectedDate; return <Pressable key={option.date} onPress={() => onSelect(option.date)} className={`min-h-20 min-w-20 items-center justify-center rounded-2xl border px-3 ${selected ? 'border-saffron-500 bg-saffron-50' : 'border-warm-200 bg-white'}`}><Text className="text-[11px] font-bold uppercase text-warm-500">{option.weekday}</Text><Text className={`text-xl font-extrabold ${selected ? 'text-saffron-700' : 'text-warm-900'}`}>{option.day}</Text><Text className="text-xs font-semibold text-warm-500">{option.month}</Text></Pressable>; })}</ScrollView></View>; }
function startOfToday() { const today = new Date(); return new Date(today.getFullYear(), today.getMonth(), today.getDate()); }
...
function toDateOnly(date: Date) { return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`; }
...
function daysBetween(startDate: string, endDate: string) { return Math.round((parseDateOnly(endDate).getTime() - parseDateOnly(startDate).getTime()) / 86_400_000); }
