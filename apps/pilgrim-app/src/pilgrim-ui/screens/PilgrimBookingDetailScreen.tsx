...
export function PilgrimBookingDetailScreen() {
      ...
      <AppScreen className="gap-6 pt-1">
        ...
        {isSyncing ? (
          <View className="items-center rounded-3xl border border-warm-100 bg-white py-16">
            ...
            <Text className="mt-4 text-sm font-bold text-warm-500">{t('Loading your booking…', 'तुमचे बुकिंग लोड होत आहे…')}</Text>
          </View>
        ) : (
          <EmptyState
            action={t('Back to my bookings', 'माझ्या बुकिंगवर परत जा')}
            body={t('This booking is unavailable or belongs to another account.', 'हे बुकिंग उपलब्ध नाही किंवा दुसऱ्या खात्याचे आहे.')}
  ...
  function askToCancel() {
    Alert.alert(
      ...
      t('Your room will be released. Refund eligibility depends on the stay policy.', 'तुमची खोली सोडली जाईल. परतावा निवास धोरणानुसार असेल.'),
      [
        { style: 'cancel', text: t('Keep booking', 'बुकिंग ठेवा') },
        {
          ...
          onPress: () => {
            void cancelBooking(bookingId, 'Cancelled by pilgrim from mobile app').catch(() => {
              Alert.alert(t('Could not cancel booking', 'बुकिंग रद्द करता आली नाही'), t('Please try again or contact Tuljai support.', 'कृपया पुन्हा प्रयत्न करा किंवा तुळजाई सहाय्याशी संपर्क साधा.'));
            });
          },
        },
      ],
    ...
    <AppScreen className="gap-6 pt-1">
      ...
      {params.justBooked === '1' ? (
        <View className="items-center rounded-3xl bg-templeGreen-50 px-5 py-6">
          <View className="h-16 w-16 items-center justify-center rounded-full bg-templeGreen-500"><MaterialCommunityIcons color="#FFFFFF" name="check" size={35} /></View>
          <Text className="mt-4 text-center text-2xl font-extrabold text-templeGreen-700">{t('Your stay is booked!', 'तुमचा निवास बुक झाला!')}</Text>
          <Text className="mt-2 text-center text-sm leading-5 text-warm-600">{t('A confirmation has been sent to your mobile number.', 'तुमच्या मोबाइल क्रमांकावर पुष्टीकरण पाठवले आहे.')}</Text>
        </View>
      ) : null}
      ...
      <View>
        <Text className="mb-3 text-xl font-extrabold text-warm-900">{t('Stay information', 'निवास माहिती')}</Text>
        <View className="rounded-3xl border border-warm-100 bg-white px-4">
          <InfoRow icon="calendar-arrow-right" label={t('Check-in', 'चेक-इन')} value={`${booking.checkIn} · after 12:00 PM`} />
          <InfoRow icon="calendar-arrow-left" label={t('Check-out', 'चेक-आउट')} value={booking.checkoutDateFlexible ? t('Not fixed · confirm with lodge', 'निश्चित नाही · लॉजसोबत ठरवा') : `${booking.checkOut} · before 10:00 AM`} />
          <InfoRow icon="bed-king-outline" label={t('Room', 'खोली')} value={booking.roomName} />
          <InfoRow icon="account-group-outline" label={t('Guests', 'पाहुणे')} value={booking.guests} last />
        </View>
      </View>

      <View>
        <Text className="mb-3 text-xl font-extrabold text-warm-900">{t('Booking journey', 'बुकिंग प्रवास')}</Text>
        <View className="rounded-3xl border border-warm-100 bg-white p-5">
          <TimelineItem active icon="check" label={t('Booking created', 'बुकिंग तयार')} subtitle={formatBookingTimestamp(booking.createdAt)} />
          <TimelineItem active={booking.status !== 'pending'} icon="home-heart" label={t('Confirmed by lodge', 'लॉजने पुष्टी केली')} subtitle={booking.status === 'pending' ? t('Waiting for response', 'प्रतिसादाची प्रतीक्षा') : formatBookingTimestamp(booking.updatedAt)} />
          <TimelineItem active={booking.status === 'checked-in'} icon="qrcode-scan" label={booking.status === 'checked-in' ? t('Checked in at lodge', 'लॉजवर चेक-इन पूर्ण') : t('Check-in at lodge', 'लॉजवर चेक-इन')} subtitle={booking.status === 'checked-in' ? t('Completed', 'पूर्ण') : booking.checkIn} last />
        </View>
      </View>

      <View className="rounded-3xl border border-warm-100 bg-white p-5">
        <View className="flex-row items-center justify-between"><Text className="text-lg font-extrabold text-warm-900">{t('Payment summary', 'पेमेंट सारांश')}</Text><Text className="text-sm font-extrabold text-templeGreen-700">{booking.paymentStatus}</Text></View>
        <View className="mt-4 flex-row items-end justify-between border-t border-warm-100 pt-4"><Text className="text-sm text-warm-500">{t('Total paid', 'एकूण भरले')}</Text><Text className="text-2xl font-extrabold text-maroon-700">{formatRupees(booking.amount)}</Text></View>
      </View>

      <View className="gap-3">
        <View className="flex-row gap-3">
          {lodge?.primaryPhone ? <SecondaryButton className="flex-1" icon="phone-outline" onPress={() => void openExternalLink(`tel:${lodge.primaryPhone}`, t)}>{t('Call lodge', 'लॉजला कॉल')}</SecondaryButton> : null}
          <SecondaryButton className="flex-1" icon="map-marker-outline" onPress={() => void openDirections(lodge?.name ?? booking.lodgeName, t)}>{t('Directions', 'दिशा')}</SecondaryButton>
        </View>
        {supportPhone || supportEmail ? <SecondaryButton icon="headset" onPress={() => void openExternalLink(supportPhone ? `tel:${supportPhone}` : `mailto:${supportEmail}`, t)}>{t('Get Tuljai support', 'तुळजाई सहाय्य मिळवा')}</SecondaryButton> : null}
