...
export async function loadBackendLodges(): Promise<PilgrimLodge[]> {
  ...
  const hydrated = await Promise.all(
    response.items.map(async (summary): Promise<PilgrimLodge | null> => {
  ...
  const availableLodges = hydrated.filter((item): item is PilgrimLodge => item !== null);

  return availableLodges.length > 0 ? availableLodges : pilgrimLodges;
}
