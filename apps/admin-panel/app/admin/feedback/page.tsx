...
export default function FeedbackPage() {
  ...
  async function moderate(id: string, nextStatus: ReviewStatus) {
    try {
      ...
      setError(
        moderationError instanceof Error ? moderationError.message : 'Could not update feedback.',
      );
    }
  }
    ...
    <div className="page-stack">
      <div className="page-header">
        <div>
          ...
          <p>Review verified stay feedback, ratings and moderation status.</p>
        </div>
      </div>
      <div className="metric-grid">
        ...
        <div className="metric-card">
          ...
          <strong>{reviews.filter((review) => review.rating === 5).length}</strong>
        </div>
      </div>
      <div className="panel-card">
        <div className="panel-header">
          <div>
            <h3>Guest reviews</h3>
            <p>Feedback submitted after completed stays.</p>
          </div>
          <select
            value={status ?? ''}
            onChange={(event) =>
              setStatus((event.target.value || undefined) as ReviewStatus | undefined)
            }
          >
              ...
              <tbody>
                {reviews.map((review) => (
                  <tr key={review.id}>
                    ...
                    <td>{review.lodgeId}</td>
                    <td>{review.pilgrimUserId}</td>
                    <td>{review.status}</td>
                    <td>
