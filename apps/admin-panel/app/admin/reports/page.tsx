...
export default function AdminReportsPage() {
  ...
  function exportBookings(): void {
    downloadCsv(
      ...
      ['Booking Code', 'Guest', 'Lodge ID', 'Check-in', 'Check-out', 'Status', 'Revenue', 'Commission'],
      ...
      <div className="page-stack">
        <section className="hero-panel">
          ...
          <div className="hero-actions">
            <button className="button button-secondary" disabled={loading || data.bookings.length === 0} type="button" onClick={exportBookings}>
              Export bookings CSV
            </button>
            <button className="button button-secondary" disabled={loading || data.commission.length === 0} type="button" onClick={exportCommission}>
        ...
        <section className="panel">
          <div className="section-header">
            ...
            <button className="button button-primary" disabled={loading} type="button" onClick={() => void loadReports()}>
          ...
          <div className="control-grid">
            <label className="form-field">
              ...
              <input max={endDate} type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} />
            </label>
            <label className="form-field">
              <span>End date</span>
        ...
        {errorMessage ? (
          <section className="error-banner">
            ...
            <button className="button button-secondary" type="button" onClick={() => void loadReports()}>
              Retry
            </button>
          </section>
        ) : null}
        ...
        <ReportTable
          ...
          rows={data.occupancy.map((row) => [row.bookingCode, row.lodgeId, row.checkInDate, row.checkOutDate, row.status])}
        />
        ...
        <div className="admin-table" role="table">
          ...
          {rows.slice(0, 500).map((row, index) => (
            <div className="admin-table-row" key={`${title}-${index}`} role="row">
              {row.map((value, valueIndex) => <span key={`${index}-${valueIndex}`}>{String(value ?? '—')}</span>)}
            </div>
          ))}
        </div>
      ) : null}
