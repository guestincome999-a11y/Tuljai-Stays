...
export class AuthService {
  ...
  public async requestOtp(
    ...
    return {
      ...
      ...(this.nodeEnv === 'development' && this.allowDevOtpResponse ? { otpForTesting: otp } : {}),
    };
  }
  ...
  public async verifyOtp(dto: VerifyOtpDto, context: RequestContext): Promise<VerifyOtpResponse> {
    ...
    const isOtpValid = this.verifySecret(dto.otp, otpRequest.otpHash);
  ...
  public async refreshToken(dto: RefreshTokenDto): Promise<RefreshTokenResponse> {
    ...
    const accessToken = await this.issueAccessToken(refreshToken.user);
  ...
  public async logout(userId: string, dto: LogoutDto): Promise<{ success: true }> {
    ...
    if (!refreshToken) {
      throw new BadRequestException('Refresh token was not found for this device');
    }
  ...
  private verifySecret(secret: string, storedHash: string): boolean {
    ...
    if (hashedSecret.length !== storedSecretHash.length) {
      return false;
    }
