...
export class ReviewsService {
  ...
  public async getBookingReview(
    ...
    return this.toReview(review, await this.getOwnerResponse(review.id));
  }
  ...
  public async moderate(
    ...
    return this.toReview(review, await this.getOwnerResponse(id));
  }
  ...
  public async respondAsOwner(
    ...
    if (response.length < 2)
      throw new BadRequestException('Response must contain at least 2 characters');
    const review = await this.prisma.review.findFirst({ where: { deletedAt: null, id } });
    if (!review) throw new NotFoundException('Review not found');
    ...
    await this.prisma.$executeRaw(
      Prisma.sql`INSERT INTO "review_owner_responses" ("review_id", "owner_user_id", "response", "updated_at") VALUES (${review.id}, ${user.id}, ${response}, CURRENT_TIMESTAMP) ON CONFLICT ("review_id") DO UPDATE SET "owner_user_id" = EXCLUDED."owner_user_id", "response" = EXCLUDED."response", "updated_at" = CURRENT_TIMESTAMP`,
    );
    await this.auditLogService.create({
  ...
  private async getOwnerResponse(
    reviewId: string,
  ): Promise<{ response: string; updatedAt: Date } | null> {
    return (await this.getOwnerResponses([reviewId])).get(reviewId) ?? null;
  }
  private async getOwnerResponses(
    reviewIds: string[],
  ): Promise<Map<string, { response: string; updatedAt: Date }>> {
    if (reviewIds.length === 0) return new Map();
    const rows = await this.prisma.$queryRaw<OwnerResponseRow[]>(
      Prisma.sql`SELECT "review_id", "response", "updated_at" FROM "review_owner_responses" WHERE "review_id" IN (${Prisma.join(reviewIds)})`,
    );
    return new Map(
      rows.map((row) => [row.review_id, { response: row.response, updatedAt: row.updated_at }]),
    );
  }
  private toReview(
    review: {
