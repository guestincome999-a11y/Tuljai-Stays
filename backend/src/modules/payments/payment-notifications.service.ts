...
export class PaymentNotificationsService {
  ...
  public async orderCreated(input: {
    bookingId: string;
    bookingCode: string;
    ...
    await this.notificationsService.create({
      ...
      recipientUserId: input.pilgrimUserId,
      title: 'Payment ready',
      type: 'SYSTEM',
    });
  }

  public async successful(input: {
    bookingId: string;
    bookingCode: string;
    pilgrimUserId: string;
    lodgeId?: string;
  ...
  public async failed(input: {
    ...
    await this.notificationsService.create({
      body: `Payment for booking ${input.bookingCode} could not be verified. No room has been confirmed. ${input.reason}`,
      bookingId: input.bookingId,
      data: { context: 'PAYMENT', operationallyImportant: true, paymentStatus: 'FAILED' },
      lodgeId: input.lodgeId,
      priority: 'HIGH',
      recipientRole: 'PILGRIM',
      recipientUserId: input.pilgrimUserId,
  ...
  public async roomUnavailableAfterPayment(input: {
    bookingId: string;
    bookingCode: string;
    pilgrimUserId: string;
    lodgeId?: string;
    ...
    await this.notificationsService.create({
      ...
      recipientRole: 'PILGRIM',
      recipientUserId: input.pilgrimUserId,
      title: 'Payment received — room unavailable',
      type: 'SYSTEM',
    });
  }
}
